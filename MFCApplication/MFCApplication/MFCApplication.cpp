...
BOOL CMFCApplicationApp::InitInstance()
{
	...
	if (nResponse == IDOK)
	{
		...
		for (int i = 0; i < dlg.infolist.size(); i++)
		{
			std::string str = CT2A(dlg.infolist[i].m_pname.GetBuffer());
			outFile << str << ','
				<< dlg.infolist[i].m_lou << ','
				<< dlg.infolist[i].m_year << ','
				<< dlg.infolist[i].m_month << ','
				<< dlg.infolist[i].m_num << std::endl;
		}
